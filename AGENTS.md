...
- Current minimum Breakout runtime is `breakout-turbo-env>=0.3.0`.
...
- When asked to tune or optimize a checked-in SB3 PPO/A2C recipe for sample efficiency and stability across training seeds, use the project-level `$autoresearch` skill in `.codex/skills/autoresearch`. It runs a bounded durable paired search, fills available `beast-3` slots with independent cohorts, confirms on five untouched seeds, and patches only the pointed leaf recipe.

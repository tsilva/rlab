...
- `stable-retro-turbo` must preserve ROM identity, save-state starts, observations, raw signals, and engine termination consistently across training, evaluation, and playback. Atari environments must use its native `AtariVecEnv` integration rather than rlab constructing ale-py's vector environment directly. Version `1.0.1.post18` is the minimum supported release because it adds the stable-retro Atari vector contract; rlab must adapt its native same-step `final_obs` output into the manual lifecycle facade without calling ale-py's deadlocking partial-reset path.
...
- SuperMarioBros-NES Turbo must match the applicable `stable-retro-turbo` public environment contract, and every provider-specific difference must be explicit and validated. Version `0.2.21` is the minimum supported release because it provides disabled autoreset and masked lane reset.
...
- Playback must support local and remote model artifacts under the evaluation preprocessing contract. Remote artifact aliases must resolve to immutable versions before caching so alias advancement cannot reuse stale model files. Visual releases must include reproducibility metadata and a representative replay.

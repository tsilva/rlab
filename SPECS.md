...
- Published policies must use the project-generated Hugging Face identity `<game-family>_<goal>_<policy-variant>_<algorithm>` under `rlab-research`; manual repository names and provider-specific repository axes are unsupported. Exact provider, environment hash, run, recipe, seed, runtime, evaluation, and artifact hashes belong in portable release metadata.

...
Promotion-quality `eval/full/*` and `candidate_stop` stages may use multiple environment lanes.
Vector evaluation samples the same stochastic policy and resolved environment contract as scalar
evaluation, so its completed episodes are valid promotion evidence even though batched random draws
do not reproduce the exact serial `rlab play` trajectories. The recorded lane count, base seed,
lane index, and per-lane episode ordinal define the reproducible `vector-lane-v1` protocol.
...
| `episode_results[].seed_protocol` | Versioned stochastic RNG trace contract. Remote checkpoint eval currently requires `vector-lane-v1`; its lane count and lane/reset ordering are part of the reproducible promotion or candidate-stop evidence. |
| `episode_results[].seed` | Scalar evaluation and playback use `base_seed + episode_ordinal`; vector evaluation records the shared base seed because lane/reset order is traced separately. Both are valid stochastic evaluation protocols. |

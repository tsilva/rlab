...
class RuntimeContractTests(unittest.TestCase):
    ...
    def test_release_descriptor_rejects_legacy_and_contract_mismatch(self) -> None:
        with self.assertRaisesRegex(ValueError, "legacy runtime artifacts"):
        ...
        payload["train_config_contract_sha256"] = "b" * 64
        with self.assertRaisesRegex(ValueError, "train-config contract mismatch"):
            runtime_refs.runtime_release_from_payload(
                payload,
                label="release",
                expected_source_sha="source",
    ...
    def test_release_descriptor_requires_exact_source_and_probe(self) -> None:
